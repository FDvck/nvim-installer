...
NVIM_VERSION=$(/usr/local/bin/nvim --version | head -n 1)
...
if [ ! -d "$HOME/.config/nvim" ]; then
  ...
  # NEW: Inject custom lua files if the custom_lua folder exists
  if [ -d "$SCRIPT_DIR/custom_lua" ]; then
    echo "=> Injecting personal NvChad configuration files..."
    cp -r "$SCRIPT_DIR/custom_lua/"* ~/.config/nvim/lua/
